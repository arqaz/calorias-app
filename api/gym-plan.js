...
export default async function handler(req, res) {
  ...
  const rl = await checkRateLimit('rl:gym-plan:' + (user ? user.sub : getClientIp(req)), 10, 3600);
  if (!rl.allowed) {
...
Responde APENAS com um objeto JSON válido, sem markdown, sem texto antes ou depois, com exatamente esta estrutura:
{
  ...
  try {
    ...
    res.status(err.status || 500).json({ error: err.message || 'Falha ao gerar o plano de treino' });
  }
}
